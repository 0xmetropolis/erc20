...
contract MetalFactory is Ownable, ERC721Holder {
    ...
    // Max WETH prebuy amounts for specific price points
    uint256 public constant MAX_WETH_PREBUY_LOW_EVALUATION = 21.7512 ether;
    uint256 public constant MAX_WETH_PREBUY_HIGH_EVALUATION = 2.13369 ether;
    ...
    function createLiquidityPool(
        ...
        if (_wethAmount > 0 && _recipient != address(0)) {
            // Select the appropriate max WETH amount based on the initial price
            uint256 maxWethAmount = _initialPricePerEth >= 0.00001 ether 
                ? MAX_WETH_PREBUY_HIGH_EVALUATION 
                : MAX_WETH_PREBUY_LOW_EVALUATION;
            ...
            uint256 amountTokensBought = _initialBuy(_token, _recipient, _wethAmount);
     ...
     */
    function _initialBuy(address token, address recipient, uint256 wethAmount)
        internal
        returns (uint256 amountTokensBought)
    {
